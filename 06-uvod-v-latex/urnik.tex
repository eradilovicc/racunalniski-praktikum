...
% \begin{center}
%    \item velikostTabele = številoUrNaDan \cdot številoDelovnihDni
% \end{center}
...
kakovost urnika:
\[ 
k_1 \cdot u_1 + k_2 \cdot u_2 + \text{...} + k_n-1 \cdot u_n-1 + kn \cdot un 
\]
...
zato si je smiselno podrobneje ogledati posamezne korake.
   ...
   \(j\)-tem koraku označimo s predavalnica\(j\).

   Za vse proste termine predavalnice\(j\) izračunamo ceno vstavljanja
   srečanja\(i\), ki ga sestavlja predmet\(_i\), profesor, ki ta predmet predava,
   ...
   Ko preverimo vse proste termine vseh predavalnic, vstavimo srečanje\(i\)
